...
function PickGame({ game, choice, onPick }) {
  return <article className="pick-game"><time>{new Date(game.startDate).toLocaleString([], { weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</time><div className="pick-options"><button className={choice === "away" ? "team-pick selected" : "team-pick"} onClick={() => onPick("away")}>{game.awayLogo && <img src={game.awayLogo} alt="" />}<span>{game.awayApRank != null && <b className="pick-rank">#{game.awayApRank}</b>}{game.awayTeam}</span></button><span className="at-symbol">@</span><button className={choice === "home" ? "team-pick selected" : "team-pick"} onClick={() => onPick("home")}>{game.homeLogo && <img src={game.homeLogo} alt="" />}<span>{game.homeApRank != null && <b className="pick-rank">#{game.homeApRank}</b>}{game.homeTeam}</span></button></div>{(game.spread != null || game.overUnder != null) && <div className="pick-lines">{game.spread != null && <span>Spread: {game.spread}</span>}{game.overUnder != null && <span>O/U: {game.overUnder}</span>}</div>}</article>;
}
