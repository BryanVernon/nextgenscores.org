...
function GameCard({ game }) {
  ...
  const localTime = start.toLocaleString()
  const homeScore = game.homePoints ?? '-'
  const awayScore = game.awayPoints ?? '-'

  return (
    <>
      <div className="game-date">Week {game.week} - {localTime}</div>
      <div className="game-info">
        <div className="game-details-left">
          <TeamBlock name={game.awayTeam} score={awayScore} logo={game.awayLogo} side="away" />
          <TeamBlock name={game.homeTeam} score={homeScore} logo={game.homeLogo} side="home" />
        </div>
        <div className="game-details-right">
          <div>{game.venue ?? 'TBD'}</div>
          <div>
            {game.spread != null && <span>Spread: {game.spread}</span>}
